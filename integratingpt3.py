...
# Configurations
# -------------------------------
download_folder = "/Users/aniketchauhan/Documents"  # Your Downloads folder path
coursera_csv_path = os.path.join(download_folder, "grades.csv")
registrar_csv_path = os.path.join(download_folder, "dummy_coursera_data.csv")  # Using the same file
...
def load_registrar_data_pd(file_path):
    ...
    try:
        ...
        # Debugging Step: Print first few rows to check if edits are being read
        print("\n✅ Checking manually edited CSV data:")
        print(df.to_string())
...
if __name__ == "__main__":
    if os.path.exists(coursera_csv_path):
        ...
        print(f"❌ {coursera_csv_path} not found. Please check the file path.")
        coursera_data = None
    ...
    try:
        if os.path.exists(registrar_csv_path):
            ...
            print(f"❌ {registrar_csv_path} not found. Please check the file path.")
            registrar_data = {}
    except Exception as e:
